...
export class CustomerController {
    ...
    @Get('customerByFirebaseId/:firebaseId')
    async getCustomerByFirebaseId(@Res() res, @Param('firebaseId') firebaseId){
        const customer = await this.customerService.getCustomerByFirebaseId(firebaseId);
        if(!customer)
            throw new NotFoundException('Customer does not exist!');
    ...
    @Get('customerImage/:fileId')
    async serveImageFile(@Param('fileId')fileId, @Res() res): Promise<any>{
